...
import akki697222.hideandseek.core.*;
import akki697222.hideandseek.core.Timer;
...
public class GameMaster {
    ...
    public static void prepare() {
        ...
        new Timer(15, bossBar, GameMaster::start, "逃げろ！ 残り%time%秒！").runTaskTimer(plugin, 0L, 1L);
    }
            ...
            if (player != null) {
                ...
                if (meta != null) {
                    AttributeModifier damageModifier = new AttributeModifier(
                            UUID.randomUUID(), "generic.attackDamage", Integer.MAX_VALUE,
                            AttributeModifier.Operation.ADD_NUMBER
                    );

                    // Multimap に追加
                    Multimap<Attribute, AttributeModifier> multimap = ImmutableMultimap.of(
                            Attribute.GENERIC_ATTACK_DAMAGE, damageModifier
                    );
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            ...
            if (isDraw) {
                ...
                if (isSeekerWin) {
                    ...
                    player.playSound(Sound.sound(org.bukkit.Sound.ENTITY_WITHER_DEATH.key(), Sound.Source.MASTER, 0.5f, 1.0f));
                } else {
                    player.showTitle(Title.title(Component.text(messageConfig.getString("game.hiderWin", "null")), Component.empty()));
                    player.playSound(Sound.sound(org.bukkit.Sound.UI_TOAST_CHALLENGE_COMPLETE.key(), Sound.Source.MASTER, 0.5f, 1.0f));
                }
            }
        });
